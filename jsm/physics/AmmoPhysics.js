async function AmmoPhysics() {
	...
	function getShape( geometry ) {
		...
		else if(geometry.type === 'BufferGeometry')
		{
			...
			return shape;
			// let triangle_mesh = new AmmoLib.btTriangleMesh();
	...
	function addMesh( mesh, mass = 0 ) {
		...
		if ( shape !== null ) {
			...
			if ( mesh.isInstancedMesh ) {
				...
				//console.log("addmesh-mesh.isMesh");
			}

		}
	}

	function change_stopLoop()
	{
		stop_loop = (stop_loop!=false) ? false : true;
	}
	...
	function handleMesh( mesh, mass, shape ) {
		...
		// 	meshMap.delete(mesh);
		// }
		if(meshMap.has(mesh)) world.removeRigidBody(meshMap.get(mesh));
	...
	function setMeshPosition( mesh, position, index = 0 ) {
		...
		} else if ( mesh.isMesh && !stop_loop) {

			const body = meshMap.get( mesh );
			//console.log("setMesh-isMesh");
			//console.log(mesh);
			//console.log(body);
			body.setAngularVelocity( new AmmoLib.btVector3( 0, 0, 0 ) );
			body.setLinearVelocity( new AmmoLib.btVector3( 0, 0, 0 ) );
	...
	return {
		...
		change_stopLoop: change_stopLoop,
		meshes: meshes,
		meshMap: meshMap
		// addCompoundMesh c
	};
